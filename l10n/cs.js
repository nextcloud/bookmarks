OC.L10N.register(
    "bookmarks",
    {
    "Bookmarks" : "Záložky",
    "Not allowed to access folder to import into" : "Není dovoleno přistupovat do složky, do které importovat",
    "Failed to import one bookmark, because: " : "Nepodařilo se importovat jednu záložku, protože:",
    "No file provided for import" : "Není k dispozici žádný soubor pro import",
    "Unsupported file type for import" : "Nepodporovaný typ souboru pro import",
    "A Bookmark manager for Nextcloud" : "Správce záložek pro Nextcloud",
    "This app provides you with a web interface for collecting and organizing bookmarks to the sites on the web that are precious to you. You can browse and filter your bookmarks via tags and folders and by using the built-in search feature. Furthermore, in order to access your bookmarks anywhere, it also allows you to synchronize third-party clients via a built-in REST API -- in your browsers and on your phone.\n\nCheck out the third-party clients listed here: https://github.com/nextcloud/bookmarks#third-party-clients" : "Tato aplikace poskytuje webové rozhraní pro shromažďování a uspořádávání záložek webových stránek, o které stojíte. Záložky je možné procházet a filtrovat pomocí štítků a složek a pomocí vestavěné funkci hledání. Dále, aby bylo možné si záložky synchronizovat kamkoli, umožňuje synchronizovat s klienty třetích stran prostřednictvím REST API – ve vašich prohlížečích a telefonu.\n\nPodívejte se na klienty třetích stran vypsané zde: https://github.com/nextcloud/bookmarks#third-party-clients",
    "Select bookmark" : "Vybrat záložku",
    "Details" : "Podrobnosti",
    "Rename" : "Přejmenovat",
    "Move" : "Přesunout",
    "Delete" : "Smazat",
    "Enter bookmark title" : "Zadejte název záložky",
    "Save" : "Uložit",
    "No bookmarks here" : "Nejsou zde žádné záložky",
    "Try changing your query or add some using the button on the left." : "Zkuste změnit svůj dotaz nebo přidat nějaké vlevo dole.",
    "Select one or more tags" : "Vyberte jeden nebo více štítků",
    "New folder" : "Nová složka",
    "Grid view" : "Zobrazení v mřížce",
    "List view" : "Zobrazení v seznamu",
    "Move selection" : "Přesunout vybrané",
    "Delete selection" : "Smazat výběr",
    "_Selected %n bookmark_::_Selected %n bookmarks_" : ["Vybrána %n záložka","Vybrány %n záložky","Vybráno %n záložek","Vybrány %n záložky"],
    "Enter a link" : "Zadejte odkaz",
    "Create" : "Vytvořit",
    "Enter folder title" : "Zadejte název složky",
    "Root folder" : "Kořenová složka",
    "_Moving %n folder and some bookmarks_::_Moving %n folders and some bookmarks_" : ["Přesouvá se %n složka a nějaké záložky","Přesouvají se %n složky a nějaké záložky","Přesouvá se %n složek a nějaké záložky","Přesouvají se %n složky a nějaké záložky"],
    "_Moving %n folder_::_Moving %n folders_" : ["Přesouvá se %n složka","Přesouvají se %n složky","Přesouvá se %n složek","Přesouvají se %n složky"],
    "_Moving %n bookmark_::_Moving %n bookmarks_" : ["Přesouvá se %n záložka","Přesouvají se %n záložky","Přesouvá se %n záložek","Přesouvají se %n záložky"],
    "New Bookmark" : "Nová záložka",
    "All Bookmarks" : "Všechny záložky",
    "Recent Bookmarks" : "Nedávné záložky",
    "Untagged" : "Neoštítkováno",
    "Import" : "Importovat",
    "Export" : "Export",
    "Sorting" : "Třídění",
    "Recently added" : "Nedávno přidáno",
    "Alphabetically" : "Abecedně",
    "Most visited" : "Nejnavštěvovanější",
    "Last modified" : "Okamžik poslední úpravy",
    "RSS Feed" : "RSS kanál",
    "This is an RSS feed of the current result set with access restricted to you." : "Toto je RSS kanál stávající sady výsledků, která bude přístupná pouze vám.",
    "Clear data" : "Vyčistit data",
    "Permanently remove all bookmarks from your account. There is no going back!" : "Trvale odstraní z vašeho účtu všechny záložky. Tato operace je nevratná!",
    "Delete all bookmarks" : "Smazat všechny záložky",
    "Bookmarklet" : "Záložky",
    "Drag this to your browser bookmarks and click it to quickly bookmark a webpage" : "Toto přetáhněte do záložek svého prohlížeče a kliknutím přidejte stránku do záložek",
    "Add to {instanceName}" : "Přidat do {instanceName}",
    "Also check out the collection of client apps that integrate with this app: " : "Také se podívejte na sadu klientských aplikací, které napojují na tuto aplikaci:",
    "Client apps" : "Klientské aplikace",
    "Do you really want to delete all your bookmarks?" : "Opravdu chcete smazat všechny své záložky?",
    "Creation date" : "Datum vytvoření",
    "Tags" : "Štítky",
    "Select tags are create new ones" : "Vyberte štítky nebo vytvořte nové",
    "Notes" : "Poznámky",
    "Sharing" : "Sdílení",
    "{time} ago" : "před {time}",
    "Select" : "Vybrat",
    "Previews" : "Náhledy",
    "In order to display real screenshots of your bookmarked websites, Bookmarks can use a third-party service to generate those." : "Pro zobrazení skutečných snímků obrazovky webových stránek, pro které jste si vytvořili záložky. Záložky mohou pro jejich vytváření použít službu třetí strany.",
    "Screeenly" : "Screeenly",
    "You can either sign up for free at screeenly.com or setup your own server." : "Je možné si buď bezplatně zaregistrovat na screenly.com nebo si vytvořit vlastní server.",
    "Screeenly API URL" : "URL adresa aplikačního program. rozhraní služby Screenly",
    "Screeenly API key" : "Klíč k aplikačnímu program. rozhraní služby Screenly",
    "Privacy" : "Soukromí",
    "Bookmarks will try to access web pages that you add to automatically add information about them." : "Záložky se budou snažit přistoupit k webovým stránkám, které jste přidali aby automaticky doplnily informace o nich.",
    "Enable accessing and collecting information from the web pages you add" : "Zapnout přístup a shromažďování informací z webových stránek, které přidáte",
    "Performance" : "Výkonnost",
    "In an installation with  a lot of users it may be useful to restrict the number of bookmarks per account." : "U instalace s mnoha uživateli může být užitečné omezit počet záložek pro jednotlivé účty.",
    "Maximum allowed number of bookmarks per account. (0 for no limit; default is no limit)" : "Nejvyšší umožněný počet záložek v jednotlivých účtech (pro bez omezení 0, výchozí je právě bez limitu)",
    "Failed to load settings" : "Nepodařilo se načíst nastavení",
    "Failed to save settings" : "Nastavení se nepodařilo uložit",
    "Add a bookmark" : "Přidat do záložek",
    "This URL is already bookmarked! Overwrite?" : "Adresa URL stánky už je v záložkách! Přepsat ?",
    "Title" : "Nadpis",
    "Link" : "Odkaz",
    "Enter bookmark url" : "Zadejte URL adresu záložky",
    "Failed to find existing bookmark" : "Nepodařilo se najít existující záložku",
    "Failed to create bookmark" : "Záložku se nepodařilo vytvořit",
    "Failed to save bookmark" : "Záložku se nepodařilo uložit",
    "Failed to move bookmark" : "Záložku se nepodařilo přesunout",
    "Failed to delete bookmark" : "Záložku se nepodařilo smazat",
    "Failed to load tags" : "Štítky se nepodařilo načíst",
    "Failed to load folders" : "Složky se nepodařilo načíst",
    "Failed to delete folder" : "Složku se nepodařilo smazat",
    "Failed to create folder" : "Složku se nepodařilo vytvořit",
    "Failed to move parts of selection" : "Nepodařilo se přesunout část výběru",
    "Failed to delete parts of selection" : "Část výběru se nepodařilo smazat",
    "Failed to fetch bookmarks." : "Záložky se nepodařilo stáhnout.",
    "Failed to change setting" : "Nepodařilo se změnit nastavení",
    "Failed to load setting " : "Nastavení se nepodařilo načíst",
    "Bookmarks feed" : "Kanál záložek"
},
"nplurals=4; plural=(n == 1 && n % 1 == 0) ? 0 : (n >= 2 && n <= 4 && n % 1 == 0) ? 1: (n % 1 != 0 ) ? 2 : 3;");
