OC.L10N.register(
    "bookmarks",
    {
    "Bookmarks" : "סימניות",
    "Failed to import one bookmark, because: " : "הייבוא של אחת מהסימניות נכשל, הסיבה:",
    "Bookm." : "סימנ.",
    "No file provided for import" : "לא סופק קובץ לייבוא",
    "Unsupported file type for import" : "סוג הקובץ לייבוא אינו נתמך",
    "Some Error happened." : "אירעה שגיאה כלשהי.",
    "Error" : "שגיאה",
    "Tags" : "תגיות",
    "Filter by tag" : "סינון לפי תגית",
    "Could not add bookmark." : "לא ניתן להוסיף סימנייה.",
    "Are you sure you want to remove this bookmark?" : "להסיר את הסימנייה?",
    "Warning" : "אזהרה",
    "Are you sure you want to remove this tag from every entry?" : "להסיר את התגית הזאת מכל רשומה שהיא?",
    "Import error" : "שגיאת ייבוא",
    "Import completed successfully." : "הייבוא הושלם בהצלחה.",
    "Uploading..." : "מתבצעת העלאה…",
    "Add a bookmark" : "הוספת סימנייה",
    "This URL is already bookmarked! Overwrite?" : "כתובת זו כבר נשמרה בסימניות! לשכתב?",
    "The title of the page" : "כותרת העמוד",
    "The address of the page" : "כתובת העמוד",
    "Description of the page" : "תיאור העמוד",
    "Save" : "שמירה",
    "Delete" : "מחיקה",
    "Edit" : "עריכה",
    "Cancel" : "ביטול",
    "Drag this to your browser bookmarks and click it, when you want to bookmark a webpage quickly:" : "ניתן לגרור את זה לסימניות הדפדפן שלך וללחוץ על זה כדי להוסיף עמוד לסימניות במהרה:",
    "Add to " : "הוספה אל",
    "Address" : "כתובת",
    "Filterable Tags" : "תגיות לסינון",
    "You have no bookmarks" : "אין לך סימניות",
    "You can also import a bookmark file" : "ניתן גם לייבא קובץ סימניות",
    "Bookmarklet" : "יישומון סימנייה",
    "Export & Import" : "ייצוא וייבוא",
    "Export" : "ייצוא",
    "Import" : "ייבוא"
},
"nplurals=2; plural=(n != 1);");
