        ...
        <AppSidebarTab v-if="!isShared" :name="t('bookmarks', 'Sharing')" icon="icon-shared">
            ...
            <div v-for="share of shares" :key="share.id">
                <div class="share">
                    ...
                    <Actions class="share__actions">
                        <ActionCheckbox :checked="share.canWrite"
                                        @update:checked="onEditShare(share.id, {canWrite: $event, canShare: share.canShare})">
                            {{ t('bookmarks', 'Allow editing') }}
                        </ActionCheckbox>
                        <ActionCheckbox :checked="share.canShare"
                                        @update:checked="onEditShare(share.id, {canWrite: share.canWrite, canShare: $event})">
	...
	export default {
		...
		methods: {
			...
			async onAddShare(user) {
				await this.$store.dispatch(actions.CREATE_SHARE, {
					folderId: this.folder.id,
					participant: user.user,
					type: user.isNoUser ? 1 : 0
				})
			},
